...
[화자 구분 코드]
녹취록에 등장하는 호칭, 이름, 발언 패턴을 분석하여 아래 코드로 자동 매핑하세요.
직함(대표이사, 부사장 등)은 회의록에 절대 명시하지 않고 코드만 사용합니다.
- [K1]: "김진호", "김대표" 등의 호칭일 때 추정
- [K2]: "김정현", "대표님" 등의 호칭일 때 추정
- [K3]: "김신근", "김부사장", "신근" 등의 호칭일 때 추정
- [S1]: "유환기", "심사역", "환기", "유심사역" 등의 호칭일 때 추정
- 호칭이나 역할이 불명확한 경우: [역할불명확] 으로 표기
- 화자 코드 옆에 직함을 괄호 등으로 부기하지 않음. 코드만 사용할 것
...
6. Q&A 포착: 발표자의 보고에 대해 다른 화자가 질의하고 답변한 내용이 있으면, 해당 주제 아래에 Q&A를 blockquote(>) 형식으로 반드시 포함
   형식 예시:
   > **Q [K2]** 질의 내용
   > **A [K1]** 답변 내용
...
4. Q&A 전수 포착: 발표·보고 내용에 대한 질의응답을 빠짐없이 포착하여 해당 안건 섹션에 Q&A를 반드시 포함. Q&A가 없는 안건이라도 논의 중 확인·추궁한 내용은 Q&A로 정리.
...
[작성 원칙]
1. 전수 포착 원칙: 녹취록에서 논의된 모든 주제를 빠짐없이 대제목(#)으로 분류하여 작성. 분량이 작거나 부수적으로 언급된 내용도 생략하지 않음. 전체 내용을 충분히 상세하게 요약할 것 (지나친 압축 절대 금지)
...
## 미팅 요약
(회의 전체 내용을 경영진 보고 수준으로 충분히 상세하게 요약. 단순 3줄이 아닌, 논의된 모든 핵심 주제·결론·수치를 포함하여 이 요약만 읽어도 회의 전체 흐름을 파악할 수 있도록 작성. 문체는 "~음" 종결)
...
■ 이 노트의 핵심 목적: 미팅에서 논의된 사실을 정확하게 기록하는 것임.
실제 질의응답을 통해 확인된 정보·판단 근거를 기록하는 것이 목적임.
추정한 내용이 있으면 반드시 *(추정)* 태그를 붙여 구분할 것.
...
[정보 출처 태그]
- 출처 태그(*(IR 자료)*, *(미팅에서 처음 확인)* 등)는 사용하지 않음
- 녹취록에서 직접 확인된 사실만 기록
- AI가 맥락상 추정한 내용이 있으면 반드시 *(추정)* 태그를 붙여 명시할 것
...
- 특히 기록해야 할 항목:
  ...
  c. AI가 맥락상 추정한 내용은 반드시 *(추정)* 태그를 붙일 것
