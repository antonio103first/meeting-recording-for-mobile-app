...
5. Q&A는 선택: 네트워킹 대화에 형식적 Q&A를 강요하지 않습니다. 화자와 질의·응답이 명백히 구분되는 의미 있는 교환이 있을 때만 Q&A로 기록하고, 없으면 Q&A 블록 자체를 생략합니다. 추정으로 Q/A를 만들어내지 마세요.
   (Q&A를 쓸 때만 적용 — v3.0.6 통일 규칙)
   - STT 원문 그대로 옮기지 말고 핵심을 한·두 문장으로 요약
   - Q와 A는 각각 별도 blockquote(>) 줄로 작성
   - **Q와 A 사이에는 빈 줄을 두지 않는다** (붙여 쓴다)
   - **A와 다음 Q 사이에만 빈 줄 1줄을 둔다** (Q&A 블록 간 구분)
...
## 회의 요약
(전체 대화를 2~3줄로 압축. 실제 확인된 사실만. 문체는 "~음" 종결)
...
(주제 핵심을 사실 그대로 서술. 내용이 많아 구분이 필요할 때만 아래처럼 **현황** / **주요 내용** 소항목을 사용하고, 짧으면 소항목 없이 단락으로 서술)
...
(주제 수에 따라 반복. Q&A는 명백한 교환이 있을 때만, 없으면 생략)
