...
6. **Q&A 대화 블록 형식 금지, 요약 서술에 통합**: `Q [코드]`/`A [코드]` 형태의 별도 blockquote 나열을 쓰지 않음. 주제별 보고 중 다른 참석자가 낸 질의·코멘트·답변은 빠짐없이 포착하되, 대화 순서 그대로 옮기지 말고 해당 주제의 서술 문단 안에 자연스럽게 녹여 정리함. 실제 질의가 불분명한데 억지로 Q&A가 오간 것처럼 창작하지 않음.
...
8. 단일 주제 내 다화자 발언 통합: 한 주제에 대해 보고자 외 다른 화자가 의견·질의·답변을 낸 경우도 화자별 줄 나열이 아닌 하나의 서술 문단으로 종합하고, 화자 표기는 문장 중간 서술어로만 녹여 인용함. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 종합함(같은 사안을 두고 여러 차례 오간 대화라도 시간순 나열이 아니라 논리 흐름 순으로 재배열).

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [K1]은 시스템이 회사 단위로 바뀌었다고 설명함. [K2]는 일부 화면이 여전히 이전 방식으로 보이는지 질의함. [K1]은 반영 직후의 일시적 현상이며 향후 통합될 예정이라고 답함. [K2]는 다른 개념과 같은 것인지 재차 질의했고, [K1]은 맞다고 답하며 세부 방식을 설명함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   [K1]은 시스템이 회사 단위로 개편됐다고 설명함. 일부 화면이 여전히 이전 방식으로 보이는 것은 반영 직후의 일시적 현상으로 향후 통합될 예정이며, 관련 개념 역시 동일하게 적용되어 세부 방식은 다음과 같음: (…)
   ```
...
5. **화자별 턴 나열 절대 금지, 하나의 서술 문단으로 종합**: `Q [화자]`/`A [화자]` 형식은 물론, `**[케이런] 의견**:` / `**[상대방] 답변**:` / `**[상대방] 제안**:` 처럼 발언자마다 줄을 나눠 나열하는 것도 금지함 — 라벨만 바뀐 대화체일 뿐임. 발표자 외 참석자의 질문·의견·이견도 빠짐없이 포착하되, 오간 순서 그대로 나열하지 말고 **줄바꿈 없는 한 문단**으로 종합 서술함. 화자 표기가 필요하면 "케이런 측에서 ~라는 의견을 냈고, 이에 상대방은 ~라고 답함"처럼 문장 중간 서술어로만 녹여 인용. STT에 실제 질문·이견이 불분명한데 억지로 대화가 오간 것처럼 창작하지 않음. **질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점·주제 단위로 재구성하여 관련 내용을 하나로 묶어 종합함**(같은 사안을 두고 여러 차례 오간 대화라도 시간순 나열이 아니라 논리 흐름 순으로 재배열).

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [케이런]은 시스템이 회사 단위로 바뀌었다고 설명함. [상대방]은 일부 화면이 여전히 이전 방식으로 보이는지 질의함. [케이런]은 반영 직후의 일시적 현상이며 향후 통합될 예정이라고 답함. [상대방]은 다른 개념과 같은 것인지 재차 질의했고, [케이런]은 맞다고 답하며 세부 방식을 설명함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   [케이런]은 시스템이 회사 단위로 개편됐다고 설명함. 일부 화면이 여전히 이전 방식으로 보이는 것은 반영 직후의 일시적 현상으로 향후 통합될 예정이며, 관련 개념 역시 동일하게 적용되어 세부 방식은 다음과 같음: (…)
   ```
...
[작성 원칙]
1. 팩트 기반: 대화에서 실제 나눈 내용에만 기초하여 작성. 녹취에 없는 내용 절대 추가 금지
2. **Q&A 대화 형식 금지, 서술로 통합**: `Q [화자]` / `A [화자]` 나열 형식을 쓰지 않음. 케이런과 상대방 사이에 오간 질의·응답·이견은 순서 그대로 옮기지 말고 **서술 문단**으로 녹여 정리함(실제 질문이 불분명한데 억지로 만들어내지 않음). 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 정리함

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [케이런]은 현금 잔고를 문의함. 상대방은 약 20억 원이라고 답함. [케이런]은 런웨이를 재차 질의했고, 상대방은 약 8개월이라고 답함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   현금 잔고는 약 20억 원으로 런웨이 약 8개월 수준임.
   ```
...
5. 주제별 요약: 각 주제는 핵심 내용을 2~3줄로 서술. Q&A 형식 사용 금지. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 정리함

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [Antonio]는 진행 상황을 문의함. [상대방]은 이번 주 중 계약이 마무리될 예정이라고 답함. [Antonio]는 대금 지급 일정을 재차 확인했고, [상대방]은 계약 체결 후 3영업일 이내라고 답함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   [상대방]은 이번 주 중 계약이 마무리될 예정이며, 대금은 계약 체결 후 3영업일 이내 지급된다고 확인함.
   ```
...
4. 주제별 압축: 각 주제는 핵심을 자연스러운 서술로 정리. 억지 구조화 금지. 오간 대화의 질문-답변 순서를 그대로 따라가며 서술하지 말고, 쟁점·주제 단위로 재구성하여 관련 내용을 하나로 묶어 정리함

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [상대방]은 최근 회사 근황을 공유함. [Antonio]는 신규 투자 유치 계획을 문의함. [상대방]은 하반기에 시리즈 B를 준비 중이라고 답함. [Antonio]는 리드 투자자가 정해졌는지 재차 질의했고, [상대방]은 아직 미정이라고 답함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   [상대방]은 하반기 시리즈 B 투자 유치를 준비 중이며, 리드 투자자는 아직 정해지지 않았다고 공유함.
   ```
...
6. **Q&A 대화 형식 금지, 서술로 통합** — 강의 중 청중과의 질의응답이 있었다면 `[Q]`/`[A]` 나열 형식을 쓰지 않고, 오간 질문·답변을 해당 소주제 서술 안에 자연스러운 문장으로 녹여 정리합니다(예: "이에 대해 한 청중이 ~라고 질문했고, 강사는 ~라고 답함"). 실제 질문이 불분명한데 억지로 만들어내지 않습니다. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점·주제 단위로 재구성하여 관련 내용을 하나로 묶어 정리합니다.

   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   강사는 개념 A를 설명함. 한 참석자가 B와의 차이를 질문함. 강사는 B는 C 관점에서 다르다고 답함. 다른 참석자가 실제 적용 사례를 질문했고, 강사는 D 사례를 소개함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   강사는 개념 A를 설명하며, B와는 C 관점에서 구분되고 실제로는 D와 같은 방식으로 적용된다고 소개함.
   ```
...
6. **패널 토론·Q&A — 대화 형식 금지, 서술로 통합** (있을 경우에만):
   ...
   예시 — 이렇게 쓰지 말 것 (❌ 질문-답변 순서를 그대로 따라간 나열, 문장으로 풀었어도 대화체와 동일):
   ```
   [발표자]는 시장 규모를 A로 제시함. [청중]은 산정 기준을 질문함. [발표자]는 B 방식으로 산정했다고 답함. 다른 [청중]이 경쟁사와의 차이를 재질문했고, [발표자]는 C 관점에서 다르다고 답함.
   ```
   이렇게 쓸 것 (✅ 쟁점 중심으로 재구성 — 확정된 결론을 먼저, 남은 질문은 그 안에 흡수):
   ```
   [발표자]는 시장 규모를 B 방식으로 산정해 A로 제시했으며, 경쟁사와는 C 관점에서 차별화된다고 설명함.
   ```
