...
8. 단일 주제 내 다화자 발언 통합: 한 주제에 대해 보고자 외 다른 화자가 의견·질의·답변을 낸 경우도 화자별 줄 나열이 아닌 하나의 서술 문단으로 종합하고, 화자 표기는 문장 중간 서술어로만 녹여 인용함. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 종합함(같은 사안을 두고 여러 차례 오간 대화라도 시간순 나열이 아니라 논리 흐름 순으로 재배열). **한 주제 안에서 질문이 3~4차례 연달아 이어지는 경우에도 예외 없이 전부 하나의 문단으로 흡수함 — 질문 개수만큼 "[K?]는 ~질의함. [K?]는 ~답함." 문장 쌍을 늘어놓지 않음.**
   ...
   ```
   [K1]은 인수 방식이 지분인수인지 질의함. [K2]는 지분인수가 아닌 자산양수도라고 답함. [K1]은 본사와의 충돌 여부를 질의함. [K2]는 본사가 한국 시장에 관심이 낮아 충돌이 없다고 답함. [K1]은 총판이 본사 자회사인지 재차 질의했고, [K2]는 자회사가 아니라고 답함.
   ```
...
[작성 원칙]
1. 팩트 기반: 대화에서 실제 나눈 내용에만 기초하여 작성. 녹취에 없는 내용 절대 추가 금지
2. **Q&A 대화 형식 금지, 서술로 통합**: `Q [화자]` / `A [화자]` 나열 형식을 쓰지 않음. 케이런과 상대방 사이에 오간 질의·응답·이견은 순서 그대로 옮기지 말고 **서술 문단**으로 녹여 정리함(실제 질문이 불분명한데 억지로 만들어내지 않음). 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 정리함. **질문이 3~4차례 연달아 이어지는 경우에도 예외 없이 전부 하나의 문단으로 흡수함 — 질문 개수만큼 "질의함. 답함." 문장 쌍을 늘어놓지 않음.**
...
5. 주제별 요약: 각 주제는 핵심 내용을 2~3줄로 서술. Q&A 형식 사용 금지. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점 단위로 재구성하여 관련 내용을 하나로 묶어 정리함. **질문이 3~4차례 연달아 이어지는 경우에도 예외 없이 전부 하나의 문단으로 흡수함 — 질문 개수만큼 "~질의함. ~답함." 문장 쌍을 늘어놓지 않음.**
...
6. **Q&A 대화 형식 금지, 서술로 통합** — 강의 중 청중과의 질의응답이 있었다면 `[Q]`/`[A]` 나열 형식을 쓰지 않고, 오간 질문·답변을 해당 소주제 서술 안에 자연스러운 문장으로 녹여 정리합니다(예: "이에 대해 한 청중이 ~라고 질문했고, 강사는 ~라고 답함"). 실제 질문이 불분명한데 억지로 만들어내지 않습니다. 질문-답변을 오간 순서 그대로 따라가며 서술하지 말고, 쟁점·주제 단위로 재구성하여 관련 내용을 하나로 묶어 정리합니다. **질문이 3~4차례 연달아 이어지는 경우에도 예외 없이 전부 하나의 문단으로 흡수합니다 — 질문 개수만큼 "질문했고, 답함" 문장 쌍을 늘어놓지 않습니다.**
