...
6. **Q&A 대화 블록 형식 금지, 요약 서술에 통합**: `Q [코드]`/`A [코드]` 형태의 별도 blockquote 나열을 쓰지 않음. 주제별 보고 중 다른 참석자가 낸 질의·코멘트·답변은 빠짐없이 포착하되, 대화 순서 그대로 옮기지 말고 해당 주제의 서술 문단 안에 자연스럽게 녹여 정리함(예: "이에 대해 [K3]는 ~라고 질의했고 [K1]은 ~라고 답함"). 실제 질의가 불분명한데 억지로 Q&A가 오간 것처럼 창작하지 않음.
...
8. 단일 주제 내 다화자 발언 통합: 한 주제에 대해 보고자 외 다른 화자가 의견·질의·답변을 낸 경우도 화자별 줄 나열이 아닌 하나의 서술 문단으로 종합하고, 화자 표기는 문장 중간 서술어로만 녹여 인용함.
...
3. **화자별 턴 나열 절대 금지 — 하나의 서술 문단으로 종합**: `Q [화자]`/`A [화자]` 형식은 물론, `**[화자] 의견**:` / `**[화자] 답변**:` / `**[화자] 제안**:` / `**[화자] 보충**:` / `**[화자] 요청**:` 처럼 발언자마다 줄을 나눠 나열하는 것도 금지함 — 라벨만 바뀐 대화체일 뿐임. 오간 의견·이견·제안·보충 설명을 전부 하나로 묶어 **줄바꿈 없는 한 문단**으로 종합 서술하고, 필요하면 "~라는 의견이 나왔고", "~라고 답함", "이에 ~라는 제안이 이어짐"처럼 문장 안에 자연스럽게 녹여 인용함(화자 표기는 문장 시작 라벨이 아니라 문장 중간 서술어로만 사용). 문단 끝에 `**결론**: ...` 한 줄로 합의·결정·보류를 명시.
