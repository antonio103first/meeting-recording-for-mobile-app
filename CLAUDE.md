# CLAUDE.md — 회의녹음요약 모바일 앱 v3.12.0

> v3.12.0 (2026-09-22): **녹음파일 바로 재생 + 카톡/슬랙 등으로 보내기 (versionCode 51, PC v4.0.5 동시 적용).** ① **재생**: 신규 `ui/components/AudioPlayer.kt` — `AudioPlayerState`(내장 `MediaPlayer` 스트리밍 재생, 화면 단위로 한 번에 하나만 재생) + `MiniAudioPlayerRow`(▶⏸·진행 슬라이더·시간). 회의목록 **MP3 탭**에서 항목을 탭하면 바로 재생/일시정지 토글되고 항목 아래 재생바 표시, 녹음/변환 화면 **파일 선택** 후 선택 파일 아래에도 미리듣기 재생바(`RecordingUiState.currentFilePath` 신설, 녹음 시작 시 미리듣기 자동 정지로 마이크 충돌 방지). 9/15에 만들다 미커밋으로 남아 있던 코드를 이어받음. ② **보내기**: 신규 `util/ShareUtil.kt` — `ACTION_SEND`+`FileProvider` URI로 시스템 공유 시트(카톡·슬랙·메일…) 호출, `content://`(SAF 가져온 파일)는 그대로 전달, FileProvider 범위 밖 절대경로는 `cacheDir/share_temp`에 복사 후 공유(IllegalArgumentException 폴백), MIME은 확장자별(mp3→audio/mpeg 등). `MiniAudioPlayerRow`에 `onShare` 파라미터 추가 → 재생바 끝 **공유 아이콘**(MP3 탭·파일 선택 미리듣기 양쪽). 공유 시트(`ShareBottomSheet`)에 **「녹음파일만 (MP3) — 카톡/슬랙 보내기」** 버튼 추가(`shareByFormat("audio_only")` → `shareAudioFile(meeting)`, 파일 없으면 비활성). 기존 「녹음포함(회의록+MP3)」은 유지. **검증**: `assembleDebug` 성공, 실기기(SM-S938N) `adb install -r` → versionCode 51 확인, 실행 후 6초 크래시 로그 없음·녹음 화면 정상. **미검증**: 실기기에서 MP3 탭 재생·공유 아이콘·카톡 전송 실주행(검증 중 폰 화면이 잠겨 조작 불가 — 회원 직접 확인 예정). versionCode 50→51 / versionName 3.11.5→3.12.0

> (2026-08-31, PC 동기화, versionCode 미변경): **Q&A 서술화 규칙 5차 재설계 — 실사용 검증 대기.** PC `Roundtable` 프로젝트 v4.0.4와 동일 세션에서 진행. 첫 Q&A 블록 폐지(아래 항목) 배포 후 회원이 실제 재요약 결과를 4차례 연속 제시하며 동일 재발 패턴(문단 앞부분은 서술로 정리되고 뒷부분 후속 확인질문 3~4개만 "[K1]은 ~질의함. [K2]는 ~답함."으로 되돌아감)을 지적. ❌/✅ 예시 추가 → 다회차 예시로 강화 → "빠짐없이 포착" 문구와의 충돌 제거 → 모두 실패 확인 후, 후처리 2차 LLM 호출안은 회원이 지연·비용 이유로 반려하고 "프롬프트를 원점에서 개선"하도록 지시. 최종적으로 겹쳐 쌓인 규칙 3~4개를 하나로 통합하고, "쟁점 중심 재구성"이라는 추상 지시를 "**화자 코드 바로 뒤에 '질의/질문/문의', '답변/답함' 동사를 붙인 문장을 아예 쓰지 마라**"는 단어 수준의 기계적 금지로 전환(7개 양식, IR 제외). "빠짐없이 기록"은 회원이 명시적으로 중요하다고 확인해 절대 약화시키지 않고 최상단에 유지. `GeminiService.kt` 동일 7개 양식 수정, `compileDebugKotlin` 검증 후 APK 재빌드·`adb install -r` 재설치까지 완료. **미해결(중요)**: 이 재설계가 실제로 재발을 막는지 아직 실사용 검증 안 됨 — 이전 4번의 시도도 처음엔 그럴듯해 보였다가 재현됐으므로, 회원이 직접 써보고 다음 세션에서 재확인하기로 함.
>
> (2026-08-31, PC 동기화, versionCode 미변경): **주간회의록(`SUMMARY_SPEAKER`) Q&A 대화 블록 전면 폐지 + 동일 화자 연속 다건 보고 통합.** PC `회의녹음요약` 프로젝트에서 실제 파트너 주간회의 사례([K2]가 모티프·PK·어피닛·에이치앤파워·GL캠·모라이 6개 포트폴리오사를 연달아 보고)를 두고 회원과 합의한 규칙을 모바일에 동일 반영(PC측 버전은 v4.0.3). ① `Q [코드]`/`A [코드]` blockquote 형식(v3.0.6부터 유지되던 것)을 제거하고, 질의·답변을 주제 서술 문단에 "이에 대해 [K3]는 ~라고 질의했고 [K1]은 ~라고 답함" 형태로 녹여 통합 — v3.7.19~v4.0.1에서 topic·formal_md·lecture_md·conference·org 5종에 이미 적용한 것을 스피커 양식에도 마저 통일(IR미팅은 예외로 미변경 유지). ② 한 화자가 여러 안건을 연달아 보고하면 안건별 소제목·화자 반복 없이 포괄적 소제목 하나 + "(보고: [K?])" 1회 + **안건명** — 내용 형식의 굵은 리드로 빈 줄 없이 나열(회원이 4가지 안 중 선택). `GeminiService.kt`의 `SUMMARY_SPEAKER` 1곳만 수정(`ChatGptService`·`ClaudeService`가 이를 참조해 자동 반영). 검증: `./gradlew compileDebugKotlin` BUILD SUCCESSFUL. **미해결**: APK 재빌드·실기기 설치 미실시(소스만 반영), 실제 STT 텍스트로 재요약해 출력 형태 육안 확인 미수행. versionCode 50 유지(빌드 없음).
>
> v3.11.2 (2026-08-12): **STT/요약 처리 중 화면 자동 꺼짐 방지 (versionCode 47).** 배경: STT 변환·AI 요약이 진행되는 동안(길면 수 분) 화면 보호기가 화면을 꺼서 처리가 중단된다는 회원 보고 — 녹음 자체는 `RecordingService`의 WakeLock으로 이미 보호되지만, 녹음 정지 후 STT/요약 단계는 화면 상태와 무관하게 백그라운드에서 도는데도 화면이 꺼지면 사용자가 중단된 것으로 오인. `MainActivity.kt`의 `MeetingRecorderApp()`에서 `RecordingViewModel.uiState.isProcessing`(STT+요약 파이프라인 전체를 감싸는 기존 상태값, 회의/음성메모/통화요약/재요약 공통)을 관찰해 `LocalView.current.keepScreenOn`을 처리 중에만 `true`로 설정(`DisposableEffect`, 완료·이탈 시 자동 해제) — `FLAG_KEEP_SCREEN_ON`과 동일 효과. **녹음 자체는 미변경**(폰 화면 설정 그대로 따름 — 회원 명시 요청). 문자열 변경 없이 Compose 바인딩만 추가라 8개 요약 양식·3개 STT/AI 엔진 전부 자동 적용. **실기기 검증**: 설치(versionCode 47) → 실제 요약 처리 중 화면 방치 후 자동 꺼짐 없이 유지 확인(회원 직접 테스트, "화면 안 꺼지고 잘 유지되네"). versionCode 46→47 / versionName 3.11.1→3.11.2
>
> v3.11.1 (2026-07-16): **티타임(네트워킹) 요약 양식 개편 (versionCode 46).** `GeminiService.SUMMARY_FLOW`(네트워킹/티타임 요약 프롬프트)의 출력 구조를 전면 개편 — 기존 `## 회의 요약 → 평면적 주제 나열(# 1, # 2 …) → Q&A 주석`을, ① **`## 한눈에`**(자리 성격 2~3줄 + 핵심 굵게 1~3개), ② **`## 주요 논의`를 맥락 3그룹**(①상대방측 사업·근황 ②케이런/본인 공유 ③사적 환담 — **실제 오간 그룹만**, 없으면 소제목째 생략), ③ **`## 후속 / 메모`**(체크박스, 없으면 생략)로 변경. **Q&A 블록 제거.** 유지: 사실충실성(Faithfulness) 원칙, 화자분리 불신, STT 오인식 `*(추정)*` 표기, 화자표기 `[Antonio]`/`[케이런]`/`[상대방]`. **세 엔진 공유** — `ClaudeService`·`ChatGptService`가 `GeminiService.SUMMARY_FLOW`를 참조하므로 요약 엔진 무관 반영. 배경: 실제 식당 티타임 녹음(듀셀 이민우 대표 골프)을 음질개선+Gemini 재전사로 정리한 요약본을 회원이 승인 → 그 형식을 앱 기본 양식으로 채택. **PC 버전(`회의녹음요약` app_dist)도 동일 적용·푸시 완료(v3.1.5, commit f4651df)** — PC·모바일 티타임 양식 통일. 문자열-only 편집(컴파일 안전). versionCode 45→46 / versionName 3.11.0→3.11.1
>
